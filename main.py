...
# giveaway_id -> {message_id, channel_id, prize, winners, host_id, end_time, entries}
...
async def enter(self, interaction: discord.Interaction, button: discord.ui.Button):
    ...
    if channel:
        try:
            ...
            await msg.edit(embed=build_giveaway_embed(
                gw["prize"], gw["winners"], gw["host_id"],
                gw["end_time"], len(gw["entries"])
            ))
        except Exception:
            pass
```
...
def **init**(self):
super().**init**(timeout=None)
btn = discord.ui.Button(label=“Giveaway Ended”, style=discord.ButtonStyle.secondary, disabled=True, emoji=“🔒”)
self.add_item(btn)
...
```
guild = bot.get_guild(int(os.environ.get("GUILD_ID", "1488422873415811092")))
if not guild:
    return
...
if num_winners == 0:
    winner_ids = []
    winner_text = "No winners — not enough entries."
else:
...
try:
    msg = await channel.fetch_message(gw["message_id"])
    await msg.edit(
        embed=build_giveaway_embed(gw["prize"], gw["winners"], gw["host_id"],
                                   gw["end_time"], len(entries), ended=True, winner_ids=winner_ids),
        ...
        if member:
            try:
                win_embed = discord.Embed(
                    ...
                    description="You have won **" + gw["prize"] + "**!\n\n⚠️ Open a ticket within the next **24 hours** to claim your prize, or it will expire!",
                    color=discord.Color.green(),
                )
...
now = datetime.utcnow()
ended = [gid for gid, gw in active_giveaways.items() if gw[“end_time”] <= now]
for gid in ended:
...
```
# Parse duration (e.g. 10s, 5m, 2h, 1d)
...
```
# Find ended giveaway by message id
if message_id is None:
    await ctx.send("Usage: !reroll <message_id>", delete_after=5)
    return

# Search history for the giveaway message
msg = None
...
if not msg:
    await ctx.send("Could not find that giveaway message.", delete_after=5)
    return

await ctx.send("Reroll not available for past giveaways without stored entries. Use !giveaway to start a new one.", delete_after=10)
```
