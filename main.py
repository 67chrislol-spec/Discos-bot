...
def **init**(self):
super().**init**(timeout=None)
self.add_item(discord.ui.Button(label=“Giveaway Ended”, style=discord.ButtonStyle.secondary, disabled=True))
...
winner_ids = random.sample(entries, num_winners) if num_winners > 0 else []
try:
msg = await channel.fetch_message(gw[“message_id”])
await msg.edit(
...
await ctx.send(“Usage: !giveaway <duration> <winners> <prize> e.g. !giveaway 1h 1 Nitro”, delete_after=10)
